...
const DateofBirthOpt = (props) => {
  ...
  useEffect(() => {
    ...
    const apiUrl = 'https://finstack-production.herokuapp.com/users/countries/';
    ...
    axiosInstance
      ...
      .then((response) => {
        if (response.data.status === true) {
          const DateofBirthOptions = response.data.data.map((bankName) => ({
            value: bankName, // Use the bank name as both the value and label
            label: bankName,
          }));

          setOptions(DateofBirthOptions);
          setIsLoading(false);
        } else {
  ...
  // const handleDateofBirthOptChange = (selectedOption) => {
  ...
  const otherDateofBirthOption = {
    value: '', // Use the value from the prop
    label: 'Other Bank',
  };

  const combinedOptions = [...options, otherDateofBirthOption];
  ...
  return (
    <div className="select-">
      <Select
        ...
        // isSearchable={false}
        onChange={handleDateofBirthOptChange}
