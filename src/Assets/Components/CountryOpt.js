...
const CountryOpt = (props) => {
    ...
    axiosInstance
      ...
      .then((response) => {
        if (response.data.status === true) {
          const CountryOptions = response.data.data.map((bankName) => ({
  ...
  // const handleCountryOptChange = (selectedOption) => {
  ...
  const otherCountryOption = {
    value: '', // Use the value from the prop
    label: 'Loading...',
  };

  const combinedOptions = [...options, otherCountryOption];
  ...
  return (
    <div className="select-">
      <Select
        ...
        // isSearchable={false}
        onChange={handleCountryOptChange}
