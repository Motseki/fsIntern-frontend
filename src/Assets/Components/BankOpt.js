...
const BankOpt = (props) => {
  ...
  useEffect(() => {
    ...
    const apiUrl = 'https://finstack-production.herokuapp.com/wallets/accounts/banks/';
    ...
    axiosInstance
      ...
      .then((response) => {
        if (response.data.status === true) {
          const bankOptions = response.data.data.map((bankName) => ({
  ...
  // const handleBankOptChange = (selectedOption) => {
  ...
  const otherBankOption = {
    value: '', // Use the value from the prop
    label: 'Other Bank',
  };

  const combinedOptions = [...options, otherBankOption];
