...
{/* </body> */}
    ...
    <main className='Aumain'>
      ...
      <section className='section'>
        <div className="lists">
          <p className="p1">Wallet Deposit </p>
          ...
          <li className={ personal ? `active` : `${stepOne}` }><span><i className="material-icons-outlined">account_balance_wallet</i> </span><p>Choose wallet { stepOne === `done` && <i className="material-icons ic">check</i>}</p> </li>
                  <li className={ idInfo ? `active` : `${stepTwo}` }><span><i className="material-icons-outlined">payment</i> </span><p>Payment method { stepTwo === `done`  && <i className="material-icons ic">check</i>}</p> </li>
                  <li className={ detailShow ? `active` : `${stepThree}` } ><span><i className="material-icons-outlined"> text_snippet</i> </span><p>Enter amount  { stepThree === `done` && <i className="material-icons ic">check</i>}</p> </li>
                  <li className={ showPreview ? `active` : `${stepFour}` } ><span><i className="material-icons-outlined">account_balance</i> </span><p>Make transfer  { stepFour === `done` && <i className="material-icons ic">check</i>}</p> </li>
          </ul>
        </div>
        <div className="cnt">
          <div className="fl1">

            { personal && <ChooseWallet
                  backclick={(e) => {
                    e.preventDefault();
                    setStepOne(`last`)
                ...
                { showPreview && <TransferDetails
                  ...
                  { success && <Seller_details_success
                    onclick={(e) => {
                      e.preventDefault();
                      // alert(`There is no where to go to!`)
                      setSuccess(true)
...
export default WalletDepositAuth